...
public class RestApiAgreements {
  ...
  private static final String PARTICIPANTSETS_ENDPOINT = "/participantSets";
  private static final String PARTICIPANTS_ENDPOINT = "/participants";
  private static final String ALTERNATE_PARTICIPANTS_ENDPOINT = "/alternateParticipants";

  private static final String PARTICIPANT_SET_INFOS = "participantSetInfos";
  private static final String PARTICIPANT_SET_ID = "participantSetId";
  private static final String PARTICIPANT_SET_MEMBER_INFOS = "participantSetMemberInfos";
  ...
  /**
   * The sender replaces the signer with an alternate signer and participant ID of new signer is returned in JSON object.
   *
   * @param accessToken Access token of the user.
   * @param requestBody Request body in JSON format containing email id of new signer.
   * @return JSON response containing the replace signer ID.
   * @throws Exception
   */
  public static JSONObject doReplaceSigner(String accessToken, String agreementId, JSONObject requestBody) throws Exception {

    // Fetch agreement info using access token and agreement ID from above.
    JSONObject agreementInfo = RestApiAgreements.getAgreementInfo(accessToken, agreementId);
    // JSONObject to get response
    JSONObject responseJSON = null;

    if (agreementInfo != null) {
      // Fetch array of participant set info using agreement info from above.
      JSONArray participantSetInfoArray = (JSONArray) agreementInfo.get(PARTICIPANT_SET_INFOS);

      if (participantSetInfoArray != null) {
        // Fetch first element of participant set info array.
        JSONObject participantSetInfoObject = (JSONObject) participantSetInfoArray.get(0);

        if (participantSetInfoObject != null) {
          String participantSetId = participantSetInfoObject.get(PARTICIPANT_SET_ID).toString();

          // Fetch array of participant set member info using participant info from above.
          JSONArray participantSetMemberInfosArray = (JSONArray) participantSetInfoObject.get(PARTICIPANT_SET_MEMBER_INFOS);

          if (participantSetMemberInfosArray != null) {
            // Fetch first element of participant set member info array.
            JSONObject participantSetMemberInfoObject = (JSONObject) participantSetMemberInfosArray.get(0);

            String participantId = participantSetMemberInfoObject.get(PARTICIPANT_ID).toString();

            // URL to replace signer.
            String url = RestApiUtils.getBaseURIForAPI(accessToken) + AGREEMENTS_ENDPOINT + "/" + agreementId + PARTICIPANTSETS_ENDPOINT
                    + "/" + participantSetId + PARTICIPANTS_ENDPOINT + "/" + participantId + ALTERNATE_PARTICIPANTS_ENDPOINT;
